...
class SinglyLinkedList:
    ...
    def insert_at_end(self, x):
        if not self.head: self.head = Node(x); return
        curr = self.head
    ...
    def delete_by_value(self, x):
        ...
        while curr and curr.data != x: prev, curr = curr, curr.next
        if curr:
...
class DoublyLinkedList:
    ...
    def insert_at_end(self, x):
        if not self.head: self.head = DNode(x); return
        curr = self.head
        while curr.next: curr = curr.next
    ...
    def insert_after_node(self, target, x):
        curr = self.head
        while curr and curr.data != target: curr = curr.next
        if curr:
...
class Stack:
    ...
    def pop(self):
        ...
        self.storage.delete_by_value(val)
        return val
    def peek(self): return self.storage.head.data if self.storage.head else None

class Queue:
    def __init__(self): self.head = self.tail = None
    def enqueue(self, x):
        new = Node(x)
        if not self.tail: self.head = self.tail = new
        else: self.tail.next = new; self.tail = new
    def dequeue(self):
...
def is_balanced(expr):
    ...
    for char in expr:
        if char in pairs.values(): s.push(char)
            ...
            if s.pop() != pairs[char]: return False
...
if __name__ == "__main__":
    ...
    sll.insert_at_beginning(3); sll.insert_at_end(3)
    ...
    for x in [10, 20, 30]: dll.insert_at_end(x)
    dll.insert_after_node(20, 25)
    print("After insert 25 after 20:"), dll.traverse()
    dll.delete_at_position(1)
    print("After deleting position 1 (20):"), dll.traverse()

    print("\n--- TASK 3: STACK & QUEUE ---")
    stk, que = Stack(), Queue()
    stk.push("A"); stk.push("B")
    print(f"Stack Pop: {stk.pop()} | Peek: {stk.peek()}")
    que.enqueue("X"); que.enqueue("Y")
    print(f"Queue Dequeue: {que.dequeue()} | Next: {que.dequeue()}")

    print("\n--- TASK 4: BALANCED PARENTHESES ---")
    for t in ["([])", "([)]"]: print(f"'{t}' -> {is_balanced(t)}")